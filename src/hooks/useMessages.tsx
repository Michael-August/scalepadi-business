...
export const useGetChats = () => {
	const { data, isLoading } = useQuery({
		...
		queryFn: async () => {
			try {
				const response = await axiosClient.get(`/chats/business`);
...
export const useCreateChat = () => {
	...
	const { mutate: createChat, isPending } = useMutation({
		mutationFn: async (data: {
			...
			try {
				const res = await axiosClient.post("/chat/business", data);
...
export const useSendMessages = (chatId: string) => {
	...
	const { mutate: sendMessage, isPending } = useMutation({
		mutationFn: async (data: { chatId: string; text: string }) => {
			try {
				const res = await axiosClient.post("/message/business", data);
				if (res.data?.status === false) {
					throw new Error(
						res.data?.message ||
