    ...
    mutationFn: async (data) => {
      try {
        ...
        if (error instanceof AxiosError) {
          ...
          toast.error(
            "An unexpected error occured while registering business."
          );
        }
    ...
    mutationFn: async (data: any) => {
      try {
        ...
        if (res.data?.status === false) {
          throw new Error(
            res.data?.message ||
              "An error occurred while setting business details"
          );
        }
        return res.data;
      } catch (error: any) {
        if (error instanceof AxiosError) {
          ...
          toast.error("An unexpected error occured while setting business.");
        }
  ...
  const { mutate: logout, isPending } = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (error instanceof AxiosError) {
          toast.error(
            error.response?.data?.message || "Failed to logout business."
          );
        } else if (error instanceof Error) {
          toast.error(error.message);
        } else {
