    ...
    mutationFn: async (data: any) => {
      try {
        ...
        if (res.data?.status === false) {
          throw new Error(
            res.data?.message || "An error occurred while setting project"
          );
        }
        return res.data;
      } catch (error: any) {
  ...
  const { mutate: hireExpert, isPending } = useMutation({
    mutationFn: async (data: any) => {
      try {
        ...
        if (res.data?.status === false) {
          throw new Error(
            res.data?.message || "An error occurred while Hiring expert"
          );
        }
        return res.data;
      } catch (error: any) {
...
export const useUpdateTaskChanges = (options?: {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      try {
        const response = await axiosClient.put(`/task/changes/${taskId}`, {
          status: status,
          note: note,
        });
        ...
        if (response.data?.status === false) {
          throw new Error(
            response.data?.message || "Failed to update task changes."
          );
        }
        // console.log(response.data);
        toast.success(
          response.data?.message || "Task changes updated successfully!"
        );
        return response.data;
      } catch (error: unknown) {
        if (error instanceof AxiosError) {
          ...
          toast.error(
            "An unexpected error occurred while updating task changes."
          );
        }
        throw error;
      }
