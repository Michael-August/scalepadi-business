    ...
    <div className="flex w-full flex-col gap-6">
        ...
        <div className="flex w-full items-center justify-between gap-4">
          <div className="top w-full flex items-center gap-3 flex-wrap">
            <div className="bg-[#D1F7FF] flex items-center justify-center p-2 text-[#1A1A1A] text-xs h-[54px] rounded-xl min-w-[80px]">
              {project?.data?.businessId?.name?.trim().split(" ")[0] || "BlueMart"}
            </div>
      ...
      {activeTab === "projectOverview" && (
        <div className="w-full border border-[#F2F2F2] rounded-2xl p-6 flex flex-col gap-6">
          ...
          {/* Goal */}
          <div className="flex flex-col gap-3">
            <h3 className="text-[#1A1A1A] text-base font-semibold">Goal</h3>
            <p className="text-sm text-[#727374] leading-relaxed">
      ...
      {activeTab === "team" && (
        <div className="w-full border border-[#F2F2F2] rounded-2xl p-6">
          ...
          <div className="grid gap-3">
            {project?.data?.experts?.map((expert: any, index: number) => (
              <div
                key={expert._id}
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-medium text-sm">{expert.id?.name}</p>
                  <p className="text-xs text-gray-500">{expert.id?.email}</p>
                </div>
                <span
        ...
        <div className="w-full border border-[#F2F2F2] rounded-2xl p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className="text-sm text-[#727374]">
              Review submissions, view feedback, and approve or request changes.
            </span>
          </div>
            ...
            {project?.data?.experts?.length > 0 && (
              <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-[#878A93]">
                    {project.data.experts.length} expert(s) assigned
                  </span>
                </div>
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex items-center gap-1 bg-[#F8F9FA] px-2 py-1 rounded-full">
                              ...
                              <span className="text-xs font-medium text-[#1A1A1A]">
                                {expertReview?.data?.averageScore || "0.0"}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4 mt-3 text-xs text-[#878A93]">
                            <div className="flex items-center gap-1">
                              ...
                              <span>
                                {expertReview?.data?.totalRatings || 0} reviews
                              </span>
                            </div>
                            <div className="flex items-center gap-1">
                              ...
                              <span>
                                Joined{" "}
                                {moment(expert?.createdAt).format("MMM YYYY")}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="mt-4">
                            <Button
                              onClick={() => handleOpenExpertReview(expert)}
                  ...
                  >
                    ...
                    {task?.link?.length > 0 && (
                      <div className="flex flex-wrap gap-2 w-full">
                        {task.link.map((link: string, i: number) => (
                          <div
                            key={i}
                    ...
                    {task?.document?.length > 0 && (
                      <div className="flex flex-wrap gap-2 w-full">
                        {task.document.map((doc: string, i: number) => (
                          <div
                            key={i}
                    ...
                    {task?.submission?.length > 0 && (
                      <div className="flex flex-col gap-3 w-full">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {task.submission.map((item: string, i: number) =>
      ...
      <Dialog open={openExpertReview} onOpenChange={handleCloseExpertReview}>
        <DialogContent className="!rounded-3xl max-w-md">
          <DialogTitle className="text-primary text-[20px] text-center">
            Rate Expert
          </DialogTitle>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3 mb-4">
                  <div className="relative w-12 h-12 rounded-full overflow-hidden">
                    <Image
                      src={selectedExpert?.profilePicture || noAvatar}
                      alt="Expert Avatar"
                      width={48}
                      height={48}
                      className="object-cover"
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#878A93]">Your Review</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <Label>Your feedback</Label>
                  <div className="rounded-[14px] py-6 px-4 border border-[#D1DAEC] bg-gray-50">
                    {userExpertRating?.note || "No feedback provided"}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <div className="text-sm text-[#878A93] text-center">
                    Review submitted on{" "}
                    {moment(userExpertRating?.createdAt).format(
                      "MMMM DD, YYYY"
                    )}
                  </div>
                </div>
              ...
              <>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#878A93]">Rate this expert</p>
                  </div>
                </div>
                ...
                <div className="form-group flex flex-col gap-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e: ChangeEvent<HTMLTextAreaElement>) =>
                      setExpertReviewDescription(e.target.value)
                    }
                    className="rounded-[14px] py-6 px-4 border border-[#D1DAEC]"
