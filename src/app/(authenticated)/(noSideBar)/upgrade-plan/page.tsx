				...
				<div className="flex flex-col gap-12 w-full py-5 lg:py-20">
					...
					{plans?.data.length === 0 && (
						<div className="flex flex-col gap-4">
							<span className="text-[#506380] text-base">
								No plans available at the moment. Please check
								back later.
							</span>
						</div>
					)}
					<div className="flex flex-col lg:flex-wrap justify-center lg:flex-row gap-6 lg:gap-10 w-full">
...
const PlanCard = ({ plan, user }: { plan: any; user: any }) => {
	...
	const componentProps = {
		...
		text: `Get ${plan?.name}`,
		onSuccess: () => {
			if (searchParams?.get("route") === "query") {
				route.push("/ai-business-query?inProgress=true");
	...
	return (
		<div className="flex flex-col gap-6 p-4 border border-[#E5E8EB80] rounded-[35px] ">
			<div
				className={`heading ${
					plan?.name.includes("Starter") && "bg-[#1A1A1A]"
				} ${
					!plan?.name.includes("Starter") && "bg-[#D9D9D9]"
				} -m-4 rounded-t-[35px] px-5 py-0`}
			>
				<h2
					className={`text-[#1A1A1A] ${
						plan?.name.includes("Starter") && "text-[#FCCE37]"
			...
			<div className="flex flex-col gap-5 py-4">
				<div className="flex flex-col gap-1">
					<span className="text-5xl text-center font-medium text-[#1A1A1A]">
						₦{plan?.cost.toLocaleString()}
					</span>
					<span className="text-lg text-center font-normal text-[#878A93CC]">
						per month
					</span>
				</div>
							...
							(benefit: string, index: number) => (
								...
								>
									<Image
										...
										height={20}
									/>
									<span className="text-sm text-[#506380] font-normal">
										{benefit}
									</span>
								</div>
							)
				...
				<PaystackButton
					...
					className={`${
						...
						plan?.name.includes("Scale") &&
						"bg-[#1A1A1A] text-white"
					} p-1 border border-[#DFDFDF] cursor-pointer rounded-[14px] hover:bg-[#E5E8EB80] hover:text-[#878A93]`}
				/>
			</div>
		</div>
	);
