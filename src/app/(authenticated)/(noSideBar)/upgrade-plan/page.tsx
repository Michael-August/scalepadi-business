...
const PlanCard = ({ plan, user }: { plan: any; user: any }) => {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 border border-[#E5E8EB80] rounded-[35px] ">
      <div
        className={`heading ${plan?.name.includes("Pro") && "bg-[#1A1A1A]"} ${
          !plan?.name.includes("Pro") && "bg-[#D9D9D9]"
        } -m-4 rounded-t-[35px] px-5 py-0`}
      >
        <h2
          className={`text-[#1A1A1A] ${
            plan?.name.includes("Pro") && "text-[#FCCE37]"
      ...
      <div className="flex flex-col gap-5 py-4">
        ...
        <PaystackButton
          {...componentProps}
          className={`${
            plan?.name.includes("Pro") && "bg-[#1E88E5] text-white"
          } ${
            plan?.name.includes("Gold") && "bg-[#1A1A1A] text-white"
